...
public class ExampleBlocks {
    public static void load() {
        new WeatherCrafter("test1") {
            {
                localizedName = "test1";
                requirements(Category.units, BuildVisibility.debugOnly, ItemStack.with(Items.copper, 25));
        ...
        new RadiusDrill("test4") {
            {
                localizedName = "test4";
                requirements(Category.production, BuildVisibility.debugOnly, ItemStack.with(Items.copper, 25));
                range = 3;
                speedPerOre = 0.2f / 60;
                updateEffect = null;
                tier = 4;
